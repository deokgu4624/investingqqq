...
const Chart1year = ({symbol, apiKey}) => {
  ...
  const close = data?.historical.slice(0, 365).map((item)=>{
    return item.close;
  })
  const date = data?.historical.slice(0, 365).map((item)=>{
    return item.date;
  })
  const areaChart = {
    series: [{
