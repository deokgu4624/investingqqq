...
const Chart6month = ({symbol, apiKey}) => {
  ...
  const close = data?.historical.slice(0, 180).map((item)=>{
    return item.close;
  })
  const date = data?.historical.slice(0, 180).map((item)=>{
    return item.date;
  })
  const areaChart = {
    series: [{
